...
### Square-root transformation for precipitation
v    <- values(clim_SA)[, 2]
logv <- sqrt(v)
values(clim_SA)[, 2] <- ifelse(is.infinite(logv), NA, logv)

### Creating PAM data for South America (run only once, then load)
# mammals <- st_read("Data/Primary Data/mammals_polygons/data_0.shp")
# mammals <- mammals %>% rename(SCINAME = SCI_NAME)
# pam_SA  <- lets.presab(mammals,
#                        xmn = -83.9361138954408, xmx = -28.8477703530605,
#                        ymn = -55.9866984156709, ymx =  13.5854011963317,
#                        res = 0.5, count = TRUE)
...
pam_mammals_sub_SA <- lets.subsetPAM(x = pam_mammals_SA,
                                     names = vale_species,
                                     remove.cells = FALSE)
...
# Excluir top_down_regulation por ter apenas 3 espécies provedoras,
# insuficiente para análise macroecológica
ES_SA <- ES_SA %>% select(-top_down_regulation)
es_names_SA <- names(ES_SA)[-1]
cat("Top-down regulation excluído. Serviços restantes:", length(es_names_SA), "\n")
...
# Map Ecosystem Services and Run GAM Models -------------------------------

es_names_SA    <- names(ES_SA)[-1]
data_plot_SA   <- list()
gam_results_SA <- list()

# Keep only non-empty environmental bins
keep_SA <- res_SA$Presence_and_Absence_Matrix_env[, 1]

# --- Residuals Diagnosis Report ------------------------------------------
pdf("Figures/Residuals_Diagnosis_Report_SA.pdf", width = 10, height = 10)
par(mfrow = c(2, 2))

for (i in seq_along(es_names_SA)) {

  # 1. Map the service in environmental space
  res_map_SA    <- lets.maplizer.env(res_SA,
  ...
  data_plot_SA[[i]] <- tibble(
    ...
    drop_na() %>%
    select(es_SA, Abundance_SA, Frequency,
           `Isolation (Mean)`, `Frequency Weighted Distance`) %>%
    pivot_longer(cols = 3:5)

  # 3. Run GAM for each predictor variable
  gam_results_SA[[i]] <- data_plot_SA[[i]] %>%
    group_by(name) %>%
    summarise(
      model = list(mgcv::gam(Abundance_SA ~ s(value),
                             data = pick(everything())))
    )

  # 4. Generate residual diagnostic plots
...
# Visualization: Split figures by service group ---------------------------

# Combine all data
out_SA <- do.call(bind_rows, data_plot_SA)

# Keep only the 3 predictor variables
out_SA_filtered <- out_SA %>%
  filter(name %in% c("Frequency", "Isolation (Mean)", "Frequency Weighted Distance"))

# Define service groups for each figure
group1 <- c("cultural_service_charismatic_species", "ecotourism",
            "disease_sentinelling", "total")
group2 <- c("pollination", "pest_and_disease_control",
            "rodent_control", "carrion_control")
group3 <- c("nutrient_transporting_horizontal_and_vertical",
            "ecosystem_engineering", 
            "seed_dispersal")

groups      <- list(group1, group2, group3)
group_names <- c("Figure1_cultural_eco_disease",
                 "Figure2_pollination_pest_rodent_carrion",
                 "Figure3_nutrient_engineering_seed_topdown")

# Generate and save each figure
for (i in seq_along(groups)) {

  p <- out_SA_filtered %>%
    filter(es_SA %in% groups[[i]]) %>%
    mutate(
      es_SA = recode(es_SA,
        ...
        "top_down_regulation"                           = "Top-Down Regulation"
      ),
      name = factor(name, levels = c("Frequency",
                                     "Frequency Weighted Distance",
                                     "Isolation (Mean)"))
    ) %>%
    ggplot(aes(y = Abundance_SA, x = value)) +
    geom_smooth(method = "gam", color = "steelblue", fill = "lightblue") +
    geom_point(alpha = 0.3, size = 0.8) +
    theme_bw() +
    facet_grid(es_SA ~ name, scales = "free") +
    labs(x = "Value", y = "Ecosystem Service Abundance") +
    theme(
  ...
  n_services <- length(groups[[i]])

  ggsave(
    filename  = paste0("Figures/", group_names[[i]], ".tif"),
    plot      = p,
    height    = n_services * 6,
    width     = 20,
    units     = "cm",
  ...
  message(paste("Saved:", group_names[[i]]))
}

# Geographic + Environmental Maps -----------------------------------------
